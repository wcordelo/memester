...
export const competitionIdFromReceipt = (receipt: TransactionReceipt | undefined): number | undefined => {
  if (receipt?.logs[2] && receipt.logs[2].topics.length === 4) {
    const competitionId = ethers.utils.hexStripZeros(receipt?.logs[2].topics[1]);

    return parseInt(competitionId, 16);
  }
};
